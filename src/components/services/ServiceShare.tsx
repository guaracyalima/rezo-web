...
export const ServiceShare: React.FC<ServiceShareProps> = ({ service, houseName }) => {
  ...
  const getShareText = () => {
    ...
    const price = `R$ ${service.basePrice.toFixed(2)}`;
    const description = service.shortDescription || service.description.substring(0, 100);
  ...
  const shareLinks = {
    ...
    twitter: () => {
      const text = encodeURIComponent(`🔮 ${service.title} - ${houseName || 'Casa Espiritual'} por R$ ${service.basePrice.toFixed(2)}`);
      const url = encodeURIComponent(getShareUrl());
